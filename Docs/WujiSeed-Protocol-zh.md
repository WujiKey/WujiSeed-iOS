...
**编码规则：**
- `processedMemory`：所有标签标准化后合并、去重、按UTF-8字节序排序、以 `0x1F`（Unit Separator）分隔拼接
- `cellIndex`：F9Grid单元格索引，8字节大端序（详见附录 C）
- **地名模式：** 当用户输入地名而非精确坐标时，cellIndex 固定为 0（8字节全零），地名作为标签参与 processedMemory 拼接
...
```
输入: ["七十二变", "筋斗云"], ["孙悟空"]
处理: 标准化 → 合并 → 去重 → UTF-8字节序排序 → 以 0x1F 分隔拼接
结果: "七十二变\x1F孙悟空\x1F筋斗云"
编码: UTF8("七十二变\x1F孙悟空\x1F筋斗云") + BE64(cellIndex)
```

**地名模式示例：**
```
输入: 地名"巴黎铁塔", 标签["蜜月", "求婚"]
处理: 地名作为标签加入 → ["巴黎铁塔", "蜜月", "求婚"] → 排序拼接
结果: "巴黎铁塔\x1F求婚\x1F蜜月"
编码: UTF8("巴黎铁塔\x1F求婚\x1F蜜月") + BE64(0)  // cellIndex = 0
```
...
所有标签排序使用 **UTF-8 字节序**（而非 Unicode 码点序或 locale 相关排序）。

**原因：** Unicode `.sorted()` 在不同平台/语言运行时中可能对 CJK 字符有不同排序行为（受 locale 影响）。UTF-8 字节序比较是纯二进制操作，在任何平台上结果完全一致。

**算法：**
```
比较两个字符串 a, b:
  1. 将 a, b 转换为 UTF-8 字节数组
  2. 逐字节比较
  3. 字节值小的排前面
  4. 若前缀相同，长度短的排前面
```

**注意：** 对于纯 ASCII 文本，UTF-8 字节序与 Unicode 排序结果一致。差异仅在 CJK 等多字节字符上可能出现。

### D.2 标签分隔符

标签拼接使用 **`0x1F`（Unit Separator）** 作为分隔符。

**原因：**
- 消除拼接歧义（不同标签组合不会产生相同的拼接结果）
- `0x1F` 是 ASCII 控制字符，不可打印，用户无法输入
- 不受标准化流程影响（normalize 不处理控制字符）
