# WujiKey: 基于可记忆信息的确定性助记词生成与门限恢复协议
...
传统加密货币钱包所依赖的BIP39助记词作为资产访问凭证，用户必须安全存储24个随机单词。这种设计面临两难困境：物理备份易丢失或被盗，而记忆随机词序列对普通用户几乎不可能。本文提出WujiKey协议，一种将用户**可记忆信息**（个人标识、地理位置、关联记忆）转换为确定性BIP39助记词的密码学方案。
