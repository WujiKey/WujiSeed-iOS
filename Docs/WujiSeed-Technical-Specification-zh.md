...
WujiSeed是一个创新的加密货币助记词生成与备份协议。它通过将用户的**个人记忆画面**（地点、事件、关键词）转化为确定性的BIP39助记词，实现"无需记忆助记词本身"的目标。本协议采用BLAKE2b、Argon2id等现代密码学原语，提供银行级安全性，支持3-of-5门限恢复（仅需3个地点即可恢复）。
...
传统助记词方案要求用户记住或安全存储24个随机单词。WujiSeed采用不同的方法：
...
**方法一：加密备份恢复（推荐）**
- 需要：加密备份文件（含方位码） + 个人标识 + 任意3个地点
- 尝试次数：方位码组合10种 × 记忆关键词组合10种 × 加密备份10个块 = 最多1000次解密尝试
...
**记忆标签处理：**
- 每个地点有两个关键词输入区域，每个区域需要1-3个关键词标签
- 推荐：每个输入区域使用1个高度个性化的关键词，以获得最强安全性
- 最少：每个输入区域1个标签（每个地点共2个标签）
- 最多：每个输入区域3个标签（每个地点共6个标签）
- 每个关键词标签单独进行标准化处理（NFKC → CaseFold → Trim → AsciiPunctNorm）
- 完成标准化处理后，把两个输入区域的所有关键词放一起，按Unicode字典序进行排序再无分隔符直接拼接
- （给出两个输入区域是为了让用户有意识地区分记忆画面，从而提高总体熵，算法内部对同一地点并不做区分）
...
```
┌─────────────────────────────────────────────────────────────────┐
│  keyMaterial = sortedBothProcessedMemory(UTF-8) +               │ 
│                cellIndex(8字节大端序)                             │
└─────────────────────────────────────────────────────────────────┘
```
...
2. **标签合并**：将两个输入区域的所有标签合并为一个数组
3. **标签去重**：移除合并后数组中的重复标签
4. **标签排序**：将所有标签按 Unicode 字典序排列
5. **拼接**：将排序后的标签无分隔符直接拼接，形成处理后的记忆字符串
6. **单元格索引**：F9Grid 返回的 Int64，转为 8 字节大端序

**示例：**
```
输入:
  memory1Tags = ["七十二变", "筋斗云"]
  memory2Tags = ["孙悟空"]
  coordinates = (35.066260, 107.614560)
...
步骤2 - 合并所有标签:
  allTags = ["七十二变", "筋斗云", "孙悟空"]

步骤3 - 去重（如果有重复标签）:
  allTags = ["七十二变", "筋斗云", "孙悟空"]

步骤4 - 按Unicode字典序排序:
  sortedTags = ["七十二变", "孙悟空", "筋斗云"]

步骤5 - 无分隔符拼接:
  processedMemory = "七十二变孙悟空筋斗云"

编码:
  keyMaterial = UTF8("七十二变孙悟空筋斗云") +
                BE64(cellIndex)
```
...
- 记忆关键词应包含**高度个人化的细节**（独特事件、个人感受、私密细节）
- **离线使用**：生成助记词时开启飞行模式
- **备份分离**：加密备份与地点提示分开存储
