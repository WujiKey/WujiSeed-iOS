...
- 尝试次数：方位码组合10种，记忆关键词组合10种，加密备份10份，解密需要完成1000次
...
- 每个关键字标签单独进行标准化处理（NFKC → CaseFold → Trim → AsciiPunctNorm）
- 完成标准化处理后，把两个输入区域的所有关键放一起，按Unicode字典序进行排序再无分隔符直接拼接
（给出两个输入区域是为了让用户有意的区分记忆画面，从而提高总体熵，算法内部同一地点并不做区分）
